...
目的：
  1. 验证国行 T90 PRO（class: jkzzec，固件 1.103.0）支持的托管相关 get* 查询
  2. MQTT 全量报文监听：抓 App 下发命令后设备的全部回推报文。
     已用此法确认托管开关 = iot/p2p/setSwitchState（agentClean: 1/0）；
     当前待解：App「智能体模式选房启动」的真实命令格式
     （HA 现在用 freeClean 短格式，启动后 App 仍显示"按我的设置清扫"）。
...
LISTEN_HINT = """
============================================================
进入 MQTT 监听阶段（%d 秒）。请按顺序操作，每步之间隔 5 秒：
  1. App【普通模式】：选 1~2 个房间 → 点启动 → 等 10 秒 → 停止
     （对照组：这次 App 会显示"按我的设置清扫"，正常）
  2. 打开「AI 智能托管」开关 → 等 5 秒
  3. 智能体模式：点选【同样的房间】 → 点启动 → 等 10 秒 → 停止
     （实验组：重点抓这次设备回推的任务参数）
  4. （可选）智能体模式直接全屋启动 → 等 10 秒 → 停止
  5. 关闭托管开关
本脚本只监听不下发命令；设备对两种启动的回推报文
（onCleanReport / onSwitchState / onAICleanItemState 等）
会全部记录，回来对比即可定位差异。
============================================================
"""
